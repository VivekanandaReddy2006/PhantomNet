...
def generate_mock_logs(n_samples=500):
    ...
    for i in range(n_samples):
        ...
        current_ip = (
            random.choice(attacker_ips) if is_attack else random.choice(normal_ips)
        )
        ...
        log = {
            ...
            "length": (
                random.randint(500, 2000) if is_attack else random.randint(50, 500)
            ),
            "protocol": random.choice(["TCP", "UDP", "ICMP"]),
            "dst_port": (
                random.choice([22, 80, 443, 2222, 8080, 21, 25])
                if is_attack
            ...
            "attack_type": (
                random.choice(["brute_force", "sqli", "scan"])
                if is_attack
                else "benign"
            ),
            "dst_ip": "172.16.0.2",
